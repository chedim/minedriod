...
public class GlyphCache
{
     ...
     * font.
     */
    private static List<Font> usedFonts = new ArrayList();
    ...
    public GlyphCache()
    {
        ...
        usedFonts.add(new Font(Font.MONOSPACED, Font.PLAIN, 1));
    }
     ...
     */
    Font setDefaultFont(String name, int size, boolean antiAlias)
    {
        Font font = null;
        for (Font used: usedFonts) {
            if (used.getFontName().equals(name) || used.getName().equals(name)) {
                font = used;
            }
        }
        if (font == null) {
            System.out.println("BetterFonts loading font \"" + name + "\"");
            font = new Font(name, Font.PLAIN, 1);
            usedFonts.add(font);
        }
    ...
    void setDefaultFont(Font font, boolean antiAlias) {
        ...
        for (Font used: usedFonts) {
            if (used.getFontName().equals(font.getFontName())) {
                isUsed = true;
                break;
            }
        }
        if (!isUsed) {
            usedFonts.add(font.deriveFont(1));
        }
