...
public class NinePatchDrawable implements Drawable {
    ...
    public void setSize(Point size) {
        ...
        if (center.x < 0 || center.y < 0) {
            throw new IllegalArgumentException("9Patch W&H should be > its border size");
        }
    ...
    public void inflate(XmlHelper node, Theme theme) {
        try {
            setDrawables((ResourceLocation) node.getResourceAttr("mc", "src", null));
            setSize(node.getSize("mc", size));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
