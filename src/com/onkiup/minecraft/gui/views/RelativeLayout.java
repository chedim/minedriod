...
import com.onkiup.minecraft.gui.primitives.CenteredRect;
...
public class RelativeLayout extends ViewGroup {
    ...
    public View.Layout measure(Point boundaries) {
        ...
        for (int i = 0; i < getChildrenCount(); i++) {
            ...
            if (oSize.x >= 0) {
                if (oSize.x > result.getInnerWidth()) {
                    result.setInnerWidth(oSize.x);
                }
            }

            if (oSize.y >= 0) {
                if (oSize.y > result.getInnerHeight()) {
                    result.setInnerHeight(oSize.y);
                }
            }
        }
